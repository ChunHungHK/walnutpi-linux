...
static void session_tracker_chk_for_sta(_adapter *adapter, struct sta_info *sta)
{
	...
	while (rtw_end_of_queue_search(phead, plist) == _FALSE) {
		...
		if (ntohs((__be16)st->local_port) == adapter->wfd_info.rtsp_ctrlport)
			op_wfd_mode |= MIRACAST_SINK;
		if (ntohs((__be16)st->local_port) == adapter->wfd_info.tdls_rtsp_ctrlport)
			op_wfd_mode |= MIRACAST_SINK;
		if (ntohs((__be16)st->remote_port) == adapter->wfd_info.peer_rtsp_ctrlport)
