...
#define STR_HDMI_HPD_FORCE "hdmi_force_hotplug"
...
static int flag_hdmi_force_hotplug=0;
static int __init parse_hdmi_force_hotplug(char *arg)
{
    if (kstrtoint(arg, 10, &flag_hdmi_force_hotplug)) {
        return -EINVAL;
    }
    printk(KERN_INFO "hdmi_force_hotplug set to %d\n", flag_hdmi_force_hotplug);
    return 0;
}

early_param(STR_HDMI_HPD_FORCE, parse_hdmi_force_hotplug);
static int sunxi_hdmi_bind(struct device *dev, struct device *master, void *data)
{
	...
	if ( sunxi_hdmi_get_hpd() || (flag_hdmi_force_hotplug == 1)) {
		_sunxi_drv_hdmi_hpd_set(hdmi, 0x1);
		_sunxi_drv_hdcp_update_support(hdmi);
	} else
		_sunxi_drv_hdmi_hpd_set(hdmi, 0x0);
